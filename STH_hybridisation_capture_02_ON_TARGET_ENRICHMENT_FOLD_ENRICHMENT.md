...
```
### On-target mapping figure, hybrid capture and WGS data 

```{r ON_TARGET_MAPPING_PLOT, fig.path='./00_FIGURES/', warning = FALSE,message = FALSE}

png(filename = "00_FIGURES/ON_TARGET_MAPPING_PLOT.png", height = 4, width = 6, units = "in", res = 300)
# Create the contamination plot to call it later
ON_TARGET_MAPPING <- ggplot(ALL_MAPPED_READS, aes(x = dataset, y = reads_mapped_percentage, fill = dataset)) +
...
print(ON_TARGET_MAPPING)
dev.off()
```
![ON_TARGET_MAPPING_PLOT](00_FIGURES/ON_TARGET_MAPPING_PLOT.png)
...
### Absolute count enrichment figure, hybrid capture and WGS data 


```{r CAPTURE_WGS_NORMALISED_READS_PLOT, warning=FALSE, fig.path='./00_FIGURES/' }

png(filename = "00_FIGURES/CAPTURE_WGS_NORMALISED_READS_PLOT.png", height = 4, width = 6, units = "in", res = 300)

CAPTURE_WGS_NORMALISED_READS_PLOT <- ggplot(CAPTURE_AND_WGS_MITO_DATA_ALUM_TT2, aes(x = dataset, y = normalised, fill = dataset)) + 
...
print(CAPTURE_WGS_NORMALISED_READS_PLOT)
dev.off()
  
```
![CAPTURE_WGS_NORMALISED_READS_PLOT](00_FIGURES/CAPTURE_WGS_NORMALISED_READS_PLOT.png)
...
### FOLD-enrichment figure, hybrid capture and WGS data 


```{r UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT, warning=FALSE, message=FALSE, fig.path='./00_FIGURES/'}

png(filename = "00_FIGURES/UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT.png", height = 6, width = 12, units = "in", res = 300) 
#PLOT 
UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT <- ggplot(UNIQUE_FOLD_ENRICHMENT2, aes(x = "", y = fold_enrichment, fill = species)) +
  geom_boxplot(outlier.shape = NA) +
...
#UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT
...
print(UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT)
dev.off()

```
![UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT.png](00_FIGURES/UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT.png)
...
png(filename = "00_FIGURES/FIG1_ON_TARGET_ENRICHMENT_FOLD_ENRICHMENT.png", height =4, width = 10, units = "in", res = 300)

ON_TARGET_ENRICHMENT <- ON_TARGET_MAPPING | CAPTURE_WGS_NORMALISED_READS_PLOT |  UNIQUE_FOLD_ENRICHMENT_ALUM_TT_PLOT +
